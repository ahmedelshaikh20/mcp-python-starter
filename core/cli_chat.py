...
class CliChat(Chat):
    ...
    async def _process_command(self, query: str) -> bool:
        ...
        # Check if command has required argument
        if len(words) < 2:
            print(f"Error: Command /{command} requires a document ID argument")
            print("Usage: /{command} <document_id>")
            print("Available documents: deposition.md, report.pdf, financials.docx, outlook.pdf, plan.md, spec.txt")
            return True
